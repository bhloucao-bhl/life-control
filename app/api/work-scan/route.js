...
const LABEL_NAME = '(Trab) processado';
const SUBJECT_RE = /^\(Trab\)\s*/i;
...
/**
 * GET /api/work-scan
 * Procura, nos e-mails recentes, o marcador "(Trab)" no início do assunto — lembretes que o
 * próprio usuário manda pra si mesmo, que podem ser tarefa ou compromisso, de trabalho ou
 * pessoal. A Claude decide tarefa x compromisso (igual ao "(Z)"); o app NÃO tenta decidir
 * trabalho x pessoal — isso sempre fica pendente pra decisão do usuário.
 */
export async function GET(req) {
  ...
  try {
    ...
    // 1a passada: metadata leve, só pra achar quais assuntos começam com "(Trab)"
    ...
    }, 6)).filter(Boolean).filter((m) => m.trabText);
...
/**
 * POST /api/work-scan  { messageId }
 * Marca o e-mail como processado (label "(Trab) processado") e arquiva (tira da INBOX) —
 * mesmo fluxo do "(Z)".
 */
