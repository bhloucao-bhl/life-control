 ...
 * Retorna null quando o resumo está desligado ou não há nada relevante pra avisar.
 */
export function buildMorningBrief(items, settings, now, notifPrefs) {
  const morning = { on: true, work: true, groceries: true, tasks: true, ...((notifPrefs || settings.notifPrefs || {}).morning) };
  if (!morning.on) return null;
 ...
 * Retorna null quando essa notificação está desligada em Ajustes.
 */
export function buildEveningReview(items, now, notifPrefs) {
  const evening = { on: true, ...((notifPrefs || {}).evening) };
  if (!evening.on) return null;
