...
export const PROVIDERS = {
  ...
  google: {
    ...
    scope: [
      // calendar.events (leitura+escrita de eventos) substitui o calendar.readonly antigo —
      // precisa pra criar convites de verdade na Agenda (com convidado, que o Google notifica por
      // e-mail). Quem já conectou antes do calendar.events existir precisa reconectar em Ajustes.
...
export async function exchangeCode(req, provider, code, user_id) {
  ...
  const row = {
    ...
    // DIAGNÓSTICO TEMPORÁRIO: só as chaves de nível 1 da resposta do token (nunca os valores),
    // pra descobrir por que o Mercado Livre não devolve refresh_token mesmo com offline_access.
    // Ver PR de investigação — remover depois de resolvido.
    token_response_keys: JSON.stringify(Object.keys(j)),
  };
